...
test_stl = validation_loader(testmanifest, root_files, be, h=im_size[0], w=im_size[1], scale=[0.875, 0.875])
...
def discriminator(inp, reuse=False):
    with tf.variable_scope('Encoder', reuse=reuse):
        ...
        conv1 = conv2d(inp, 64, kernel=3, strides=1, name=dname + 'conv1')
        conv1 = lrelu(conv1, 0.2)

        conv1b = conv2d(conv1, 64, kernel=3, strides=2, name=dname + 'conv1b')
        conv1b = batchnorm(conv1b, is_training=is_train, name=dname + 'bn1b')
        conv1b = lrelu(conv1b, 0.2)
        # 32
        conv2 = tf.nn.dropout(conv1b, keep_prob)
        conv2 = conv2d(conv2, 128, kernel=3, strides=1, name=dname + 'conv2')
        conv2 = batchnorm(conv2, is_training=is_train, name=dname + 'bn2')
        conv2 = lrelu(conv2, 0.2)

        conv2b = conv2d(conv2, 128, kernel=3, strides=2, name=dname + 'conv2b')
        conv2b = batchnorm(conv2b, is_training=is_train, name=dname + 'bn2b')
        conv2b = lrelu(conv2b, 0.2)
        # 16
        conv3 = tf.nn.dropout(conv2b, keep_prob)
        conv3 = conv2d(conv3, 256, kernel=3, strides=1, name=dname + 'conv3')
        conv3 = batchnorm(conv3, is_training=is_train, name=dname + 'bn3')
        conv3 = lrelu(conv3, 0.2)

        conv3b = conv2d(conv3, 256, kernel=3, strides=2, name=dname + 'conv3b')
        conv3b = batchnorm(conv3b, is_training=is_train, name=dname + 'bn3b')
        conv3b = lrelu(conv3b, 0.2)
        # 8
        conv4 = tf.nn.dropout(conv3b, keep_prob)
        conv4 = conv2d(conv4, 512, kernel=3, strides=1, name=dname + 'conv4')
...
with tf.Session(config=config) as sess:
    ...
    for i_iter in range(init_iter, max_iter):
        ...
        _, lossDn, lossOn, lossFake = sess.run([d_optimizer, cost_Dn, cost_On, cost_Dg_fake], feed_dict={
            x_n: batch_x_stl, y_stl: batch_y_stl,
